...
public class Account {
    ...
    public void withdraw(double amount, String ssn) throws NegativeAmountException, InsufficientBalanceException, SsnNotValidException {
        try {
            if (amount < 0) throw new NegativeAmountException("The amount= " + amount + " be positive.");
            if (amount > balance) throw new InsufficientBalanceException("The balance= " + balance + " is insufficient.");
            if (!isSsValid(ssn)) throw new SsnNotValidException("The SSN= " + ssn + " does not match.");
            balance -= amount;
    ...
    /**
     * Returns the current of the account.
     * @return  the current of the account.
     */
    ...
    /**
     * Checks if the input SSN is the same with the system SSN
     * @param ssn   The system SSN.
     * @return      True if they are equal, false if not.
     */
    private boolean isSsValid(String ssn) {
